# ACE-GF – Behavior Verification Test Suite  
*A fully offline, air-gapped, stateless cryptographic behavior testing toolkit.*

## ✨ ACE-GF Capabilities Overview

| ACE-GF Capability | Technical Description |
| :--- | :--- |
...
| **Legacy Asset Assimilation** | Existing cryptographic material (e.g., private keys, symmetric keys, or legacy seeds) from Key Management Systems (KMS), Hardware Security Modules (HSM), or traditional wallets can be securely assimilated into the ACE-GF Crypto Entity model, leveraging its next-generation security features. |
...
because its purpose is to demonstrate the behavioral guarantees of ACE-GF.

The production ACE-GF Core library will enforce:
...
> were compiled from an **older** version of the ACE-GF implementation and  
> do **NOT** reflect the algorithms, constants, sealing scheme, or  
> domain-separated HKDF structure described in the **latest ACE-GF paper**.  
...
The ACE-GF project has recently added several major improvements:

### 1. Enterprise-grade Revocation Layer (New)
A new revocation primitive has been added to ACE-GF Core.  
...
ACE-GF can now wrap and assimilate existing legacy private keys  
...
The original ACE-GF / ADI design paper is now included in this repository  
...
### macOS
```bash
sudo dtruss -f ./acegf generate "test123"

### Linux
```bash
strace -f -e trace=file,network ./acegf generate "test123"
```
...
## 3.1 Scenario 1 — Generate and view the Same Crypto Entity from a Mnemonic
...
```bash
./acegf view "pass" "<mnemonic>" 
```

### Step 2 — Rotate passphrase

```bash
./acegf rekey "oldpass" "newpass" "<mnemonic>" 
```

This produces a new mnemonic corresponding to the new passphrase.

### Step 3 — view with the new passphrase

```bash
./acegf view "newpass" "<new_mnemonic>" 
```
...
### 3.3 Scenario 3 — Multi-Version Passphrase
...
```bash
./acegf view "P1" "<mnemonic_1>" 
./acegf view "P2" "<mnemonic_2>" 
./acegf view "P3" "<mnemonic_3>" 
```
`
**Expected:**
...
**Step 3 — view**
```bash
./acegf view "<passphrase>" "<mnemonic>" 
```
...
The ACE-GF Behavior Verification Playground is released under MIT License.
